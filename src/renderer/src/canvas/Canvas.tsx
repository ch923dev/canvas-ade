...
function CanvasInner(): ReactElement {
  ...
  // M8: gate buildDigest (O(boards×elements)) on digestOpen — reuse the last digest via STATE
  // (not a ref: refs can't be read during render) while closed. `setLastDigest` mirrors the
  // "adjust state during render" pattern above (digestProjectKey/digestOpen): safe, terminates
  // once digest === lastDigest.
  const [lastDigest, setLastDigest] = useState(EMPTY_DIGEST)
  const digest = useMemo(
    () =>
      digestOpen ? buildDigest({ schemaVersion: SCHEMA_VERSION, boards, connectors }) : lastDigest,
    [boards, connectors, digestOpen, lastDigest]
  )
  if (digestOpen && digest !== lastDigest) setLastDigest(digest)
