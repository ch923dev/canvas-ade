...
| 002 | ⛔ not achievable (closed) | **Premise disproven (2026-06-23).** "Transferable ArrayBuffer, zero-copy" only works **within** a process (renderer↔worker — already done by 006), NOT across the main→renderer **process boundary**: verified against Electron 42 typings, *every* `postMessage`/`send` transfer list is `MessagePort[]`/`MessagePortMain[]` — **none accepts an ArrayBuffer** — so a `MessageChannelMain` rewrite would still structured-clone (copy) the buffer while adding a security-sensitive preload port surface for zero gain. The cross-process copy of a CPU bitmap is **irreducible**. SLICE-005 (crop) already made the copy KB-sized for the common case; only **full-repaint video/animation** still pays the full copy, which is fundamental. The sole true zero-copy path is **shared-texture OSR** (`useSharedTexture` → GPU handle) = a WebGL/GPU pipeline rewrite (OSR "OS-4" tier), **not a slice**. No code shipped; rationale in `slices/SLICE-002.md`. |
...
| 010 | ✅ fixed | **Wave 2** — the OsrNetworkPanel request table is now windowed. A `react-window` rewrite was rejected (it is only a TRANSITIVE dep of react-arborist — a worktree can't `pnpm install` to declare it — and the div-grid conversion would risk the "column layout + waterfall identical" invariant). Instead a table-preserving spacer-row virtualization (`virtualRows.ts`: pure `computeRowWindow` + `useVirtualRows` hook — scroll/resize observers, rAF-coalesced, runtime row-pitch measure) renders only the viewport (+overscan) rows between two spacer `<tr>`s; the real `<table>`/`<td>`/waterfall stay byte-identical. `Row` is now `React.memo`'d with a stable `onClick` (`useCallback`) + `wfWin` (pipeline memo) so it holds across selection/scroll re-renders. At the 1000-record cap ~30 `<tr>`s mount instead of 1000. `virtualRows` unit (8/8, incl. the topPad+rendered+bottomPad===total*rowH scroll-height invariant + stale-scrollTop clamp); **live e2e** (`browserNetwork` 7/7 incl. a new SLICE-010 guard: seed 1000 via a new `seedOsrNet` e2e seam → assert <80 `<tr>` mount, the last record is absent until a scroll-to-bottom mounts it, filter still resolves to the single match). BrowserBoard chunk unchanged at 109 KB (no react-window chunk → zero new dep). **Fixed a latent attach-after-mount bug:** the panel returns null until open, so the hook's listeners must (re-)attach on a `board.open` dep flip (else scroll never re-windows). |
...
- **Heavy live preview (4 Browser boards @ S=2, 30 fps):** ~0.58 core reclaimed (006 swizzle off the
  main thread) and ~2 GB/s of short-lived GC churn removed (005); small paints (caret/scroll) drop
  from 16.4 MB/frame to ≈KB (005). **002 (the ~0.5-core MAIN→renderer copy) is NOT recoverable** —
  the cross-process IPC copy is irreducible for a CPU bitmap (see `slices/SLICE-002.md`); 005 already
  shrinks it to KB for everything but full-repaint video/animation, where it is fundamental.
