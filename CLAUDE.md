...
- **Start here next session — VERIFY LIVE + Phase 3.** Run `pnpm dev`, add each board from the dock, and exercise what static capture couldn't: Terminal type/resize/interrupt/restart; Browser native-view alignment + reflow through zoom + URL reload + load-failed on a real localhost; Planning note/checklist CRUD + toggle + pen-under-cursor at varying zoom + drag/resize. Then Phase 3: persistence (`canvas.json` file I/O + autosave), Focus/Full view, Duplicate, git worktrees + per-board ports. The 3 agent worktrees remain under `.claude/worktrees/` (locked, gitignored) — remove with `git worktree remove -f -f` if desired. Phase 1 handoff complete.
