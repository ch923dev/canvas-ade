...
- Next: **Phase 1 GATE** finishes — step **1-E** (see `docs/roadmap.md`): refactor `preview.ts` single-view → keyed multi-view PreviewManager (`Map<boardId, WebContentsView>`), 2+ views from one rAF batch, responsive reflow at W∈{390,834,1280} via `setZoomFactor`+`setBounds`, cap ~4 live, `webContents.close()` leak check. Then the **🚦 GATE verdict**. Do not proceed past the gate if janky/leaky.
- **Start here next session:** `docs/handoffs/phase-1.md` (cold-start handoff; **1-E is the immediate next step**).
