...
- **Start here next session:** `docs/handoffs/phase-2.md` → **step 2.0-D** (app chrome shell: floating dock `▦ select | + Terminal | + Browser | + Planning`, top-right camera cluster, top-left project switcher, empty state, keys, dbl-click focus + dimOnFocus — replaces the TEMP add-bar/seed in `Canvas.tsx` and the `App.tsx` tab harness). **2.0-D COMPLETE → parallel board work (2.1/2.2/2.3) may begin.** Phase 1 handoff (`docs/handoffs/phase-1.md`) is complete.
