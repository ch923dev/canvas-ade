...
- **Start here next session:** `docs/handoffs/phase-2.md` → **step 2.0-C** (real canvas + shared `BoardFrame` + restyled `NodeResizer` + selection/LOD; promote the salvaged PreviewManager algorithm from `smoke/FlowSmoke.tsx`). Consumes the 2.0-B schema + store. Phase 1 handoff (`docs/handoffs/phase-1.md`) is complete.
