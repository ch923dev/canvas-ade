...
test('flip shows the two-zone recap for a terminal board', async ({ page, electronApp }) => {
  ...
  try {
    ...
    const wrote = await mainCall<boolean>(electronApp, 'writeRecapJson', id, NARRATIVE)
 ...
 */
test('facts render from the transcript with no LLM key', async ({ page, electronApp }) => {
