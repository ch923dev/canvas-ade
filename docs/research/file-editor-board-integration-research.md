...
A board type is a discriminated-union member in `src/renderer/src/lib/boardSchema.ts`: `BoardType = 'terminal' | 'browser' | 'planning'`, each extending `BoardCommon { id, type, x, y, w, h, title, z? }`. Adding a type is a fixed recipe: (a) add the literal to `BoardType` + an interface to the `Board` union; (b) add it to `DEFAULT_BOARD_SIZE`, `DEFAULT_TITLE`, and the `createBoard()` factory switch; (c) add a `case` to `assertBoard()` (and an element validator if it has sub-content); (d) bump `SCHEMA_VERSION` (currently **4**) and add a no-op/backfill entry to `MIGRATIONS`; (e) whitelist patchable fields in `PATCHABLE_KEYS` in `canvasStore.ts` (~line 266) — only those keys survive `updateBoard()`; (f) add a `board.type === '…'` branch in `BoardNode.tsx` (a `lazy()` code-split chunk) plus a `TYPE_TAG`/glyph entry in `BoardFrame.tsx` + `TypeGlyph.tsx`.
