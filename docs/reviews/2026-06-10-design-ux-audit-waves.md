...
| D4 | ✅ merged (all 3 lanes; umbrella complete 2026-06-12) — **A merged** #121 `b19fc41` (2026-06-11, FIRST in wave): 🎨 command palette (Ctrl+K) + `?` shortcuts view, spec + 3-state mock signed off pre-impl (Ctrl+K fires while typing in canvas inputs, xterm exempt; `--scrim` reused). One island on the D1-B Modal: searchable verbs w/ shortcut chips (create/goto/rename/duplicate/delete/full-view · terminal restart resume-gated · planning export via shared `runExport.ts` · group ops · tidy/fit/reset · undo/redo rail-gated) + the full shortcuts sheet. One registry feeds both views + a chip↔`resolveCanvasKeyAction` drift-guard test; Esc layering = confirm gate → `[data-palette-open]` → full view (BUG-005 intact); ADR 0002 token-keyed detach; intent channel for rename/restart; ratchet paid via `hooks/useBoardActions.ts` extraction (Canvas 757/779). 3 bot rounds / 5 threads (4 fixed incl. view-conditional ARIA, 1 declined w/ reasoning); unit 2141, e2e 84×2 ×3 runs + post-merge. **B merged** #123 `0b3bb1a` (2026-06-11, 2nd in wave): keyboard-first canvas — **closes A3 + A4, the last two High a11y findings**. Tab/Shift+Tab cycle boards in spatial reading order (wrap + scroll-into-view, exits focus mode) · arrows move 1px/Shift 10px · Alt+arrows resize (MIN clamp, no phantom step) — key-repeat burst = ONE undo step (#119/#94 grammar) · Enter = the double-click camera-fit (onNodeDoubleClick now delegates to the same focusBoardById) · A3: Esc in a focused native preview returns OS focus to the host webContents (append-only before-input-event extension; renderer selects the board, existence-gated; all 6 preview invariants untouched). RF's built-in node keyboard a11y DISABLED (its arrow-move had NO undo checkpoint; tabIndex walked DOM order); gating = whitelist guard `shouldFireBoardNavKey` (body/pane focus only — xterm, planning well, inputs, Modal/Menu traps, chrome all keep native keys). New hooks/useBoardKeyboardNav.ts (Canvas ratchet net-negative); 5 new ?-sheet rows drift-guarded. 1 bot round / 3 threads all fixed `f460aba` + dispositioned, r2 clean; unit 2166, e2e 91×2 ×3 runs + post-merge. **C merged** #124 `48bc8c3` (2026-06-12, LAST in wave): 🎨 wayfinding minimap island — both wireframes (minimap vs board list) presented side-by-side, USER PICKED the minimap, toggled + remembered (board list declined: palette-goto already covers named scan; the gap is spatial, and §8 lists the minimap as the optional island). RF `<MiniMap>` first import, token-themed via `--xy-minimap-*` (surface-raised box, border-strong rects, accent viewport ring + selected rect, 120ms reduced-motion-gated fade); board-rect click = the D4-B `focusBoardById` path (deferred one macrotask past the pannable d3 gesture), empty-map click = teleport at current zoom; bare `M` + palette verb + ?-row, drift-guarded; sticky `wayfindingStore` (localStorage); ADR 0002 — the island's live rect joins `resolveChromeZones` + a `minimapVisible` liveness dep, toast island lifts above it; NOT an Esc layer. 2 rode-along root-cause fixes its own e2e surfaced: `boardNodes` `initialWidth/Height` (RF `measured` never sticks to rebuilt user nodes — minimap rendered zero rects) and the empty-note autofocus-vs-native-mousedown-focus race (note died the instant the note tool placed it; autofocus deferred one macrotask, the D1-C discipline). Bot review round 1 CLEAN (zero inline). unit 2187, e2e matrix Win 100/100 + Linux 99+1 skip-by-design. **🏁 WAVE D4 COMPLETE — THE 2026-06-10 DESIGN/UX AUDIT UMBRELLA IS COMPLETE** (D0 #108 · D1 #111/#112/#113 · D2 #114/#115/#116/#117 · D3 #118/#119/#120 · D4 #121/#123/#124). |
