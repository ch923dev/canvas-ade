...
| D2 | in progress — **A merged** #114 `0a45583` (2026-06-11): inline title edit (double-click + F2, Enter/blur commit, Esc cancel, one undo gesture; §6 mandate closed; unit 1995, e2e 67×2). **B merged** #116 `caf8212` (2026-06-11): terminal polish — config unsaved-changes guard · spawning sliver (`BoardFrame.spawning`) · restart menu on the shared Menu shell · 🎨 first-run launchCommand hint (signed off) · A6 flip-focus transfer; 3 real bugs caught by its own e2e pre-land (hint-pill link-layer occlusion, A6 focus no-op, A6 mid-fold click swallow); unit 2020, e2e 71×2. C in flight (#117 ready); **D motion merges LAST** (shares `BoardFrame.tsx` with A → D rebases). |
| D3 | not started |
