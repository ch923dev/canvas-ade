...
**One open RECOMMENDATIONS round** (not a remediation backlog): the **2026-06-23 in-depth MCP feature
audit** — 26 findings + a curated 7-skill / 14-command recommendation set with a P0–P2 build sequence;
*recommendations not yet scoped into PRs* (see the table). All **remediation** rounds below are CLOSED.
The **2026-06-19 feature-improvement audit** (the prior open round) is
...
| # | File / status |
|---|---|
| In-depth — MCP feature audit (2026-06-23) | [`2026-06-23-mcp-audit/`](2026-06-23-mcp-audit/) — full audit of the MCP / agent-orchestration layer (agent-facing **functions** · human-facing **commands** · the net-new **"skills"** concept) via an 18-agent Map→Audit→Design→Verify→Synthesize workflow. **26 findings** (0 Crit · 4 High · 13 Med · 9 Low — **zero security-invariant violations**) + a critic-deduped recommendation set of **7 skills + 14 commands** with a P0–P2 build sequence. Centrepiece decision: a "skill" = a tier-gated **MCP-prompt playbook** (the empty `registerPrompts` stub is the natural home), backed by a CLI-agnostic agent primer + human recipe templates. [`REPORT.md`](2026-06-23-mcp-audit/REPORT.md) + machine-readable [`proposals.json`](2026-06-23-mcp-audit/proposals.json) + a **spec-driven-development package** [`sdd/`](2026-06-23-mcp-audit/sdd/) (master plan + 7 executable Wave-1 specs + Wave-2/3 cards, partitioned for parallel sessions). **OPEN — recommendations not yet actioned.** |
