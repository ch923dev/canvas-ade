...
**No open review backlog.** The **2026-07-13 Jarvis epic review** (the newest round) is
**COMPLETE** — all findings (2 critical · 8 major · 13 P2 · 2 nits) fixed or dispositioned across
the review wave (#343 + #350), J4 (#352, BRAIN-5 injection audit) and J5 (#354, deferred tail);
summary: [`2026-07-13-jarvis-epic-review.md`](2026-07-13-jarvis-epic-review.md). Before it, the
**2026-07-02 bug-hunt expedition** is
**COMPLETE** — all **57 confirmed findings (1 Crit · 9 High · 28 Med · 19 Low)** fixed and merged to
`main` via umbrella **PR #279** (`729bdde0`, 2026-07-02); package:
[`2026-07-02-bug-hunt-expedition/`](2026-07-02-bug-hunt-expedition/). Before it, the
**2026-06-19 feature-improvement audit** is likewise
...
| # | File / status |
|---|---|
| Jarvis epic review (2026-07-13) | [`2026-07-13-jarvis-epic-review.md`](2026-07-13-jarvis-epic-review.md) — four parallel deep passes over the full J-epic diff vs `main` (umbrella @ `ce1323be`, 64 files +9.7k): **2 critical (MIC-1 hot mic behind a closed panel · TTS-1 download stream error = whole-app exit) · 8 major · 13 P2 · 2 nits**. Defects clustered in two seams: single-shot precondition checks on multi-step async transitions, and the filesystem/crash boundary. **✅ COMPLETE** — P0+P1+cheap P2s fixed in the review wave (#343 + #350, 0.17.1, regression tests each), BRAIN-5 closed by the J4 injection audit (#352), deferred tail (BRAIN-3 · TTS-5/6/7 · MIC-3 · BADGE-1 · PANE-1 · E2E-1) closed in J5 (#354). Raw cards collapsed to git history (`docs/reviews/2026-07-13-jarvis-epic-review/`). |
| Terminal copy/paste research (2026-07-11) | [`2026-07-11-terminal-copy-paste-research/`](2026-07-11-terminal-copy-paste-research/) — deep research (12-agent workflow: repo recon + 6 web angles + gap round, all claims verified at shipped versions xterm 5.5.0 / @xyflow/react 12.11.0) into copy/select failing while a CLI agent streams. **6 ranked root causes**: agent DECSET mouse-tracking → `SelectionService.disable()` wipe · our Ctrl+C SIGINT fallthrough on a wiped selection · coords-not-text stale copies under Ink redraws · fire-and-forget clipboard write + unconditional clear · scrollback-trim destruction · React Flow `selectionKeyCode:'Shift'` swallowing the Shift+drag escape hatch. **✅ P0+P1 fixed in the same PR** (`fix/terminal-copy-selection`): selection snapshot fallback + no-SIGINT-after-failed-copy + verified clipboard write + `selectionKeyCode:null` + `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1`/`FORCE_HYPERLINK=1` spawn env. P2 follow-ups (copy-on-select option, coalescer hold-during-drag, link copy menu, dblclick-flip) live in [`RESEARCH.md`](2026-07-11-terminal-copy-paste-research/RESEARCH.md) § Fix plan. |
| Bug-hunt expedition (2026-07-02) | [`2026-07-02-bug-hunt-expedition/`](2026-07-02-bug-hunt-expedition/) — full-repo adversarial hunt: **57 confirmed in-scope (1 Crit · 9 High · 28 Med · 19 Low)**. Each bug fixed by a dedicated subagent in its own isolated worktree (`fix/BUG-NNN-*`, 5 parallel waves), merged one-by-one into `integ/cumulative` with central conflict reconciliation (both intents preserved), central gate green (typecheck · lint 0-err · format · 4064 unit/integration). **✅ COMPLETE — all 57 fixed**, landed on `main` via umbrella **PR #279** (`729bdde0`, 2026-07-02). Crit = MCP confirm-dialog Unicode bidi/invisible-char sanitization (BUG-001). ⚠️ Package note: per-finding card bodies BUG-002–057 were lost mid-run to a fix-agent `git clean` (INDEX reconstructed verbatim; fixes unaffected; agent prompt hardened since). [`INDEX.md`](2026-07-02-bug-hunt-expedition/INDEX.md) · [`FIX-REPORT.md`](2026-07-02-bug-hunt-expedition/FIX-REPORT.md). |
| gitEnv `SSH_ASKPASS` seam fix (2026-06-29) | [`2026-06-29-gitenv-ssh-askpass/`](2026-06-29-gitenv-ssh-askpass/) — single-fix: `repoScopedEnv()` stripped every `GIT_*` var but missed `SSH_ASKPASS` (an OpenSSH var, no `GIT_` prefix), so `simple-git`'s `blockUnsafeOperationsPlugin` still refused to spawn → broken `gitDiff`/`gitPermalink` for any user whose shell exports it (e.g. Git Bash). **✅ FIXED** on `fix/gitenv-ssh-askpass`: explicit `delete env.SSH_ASKPASS` (+ companion), guard kept (no `allowUnsafeAskPass`). e2e contrast proven (un-fixed FAILS / fixed PASSES with `SSH_ASKPASS` set). Summary: [`SUMMARY.md`](2026-06-29-gitenv-ssh-askpass/SUMMARY.md). |
| Performance wave (2026-06-23) | [`2026-06-23-perf-wave/`](2026-06-23-perf-wave/) — measurement-grounded perf review (8 parallel discovery zones → adversarial verify per candidate): 43 candidates → 15 confirmed → **13 ROI-ranked slices**, targeting the File Tree / File Board / Command Board white space + the OSR frame pipeline. **✅ COMPLETE** — **12 shipped + SLICE-002 closed as not-achievable** (no cross-process ArrayBuffer transfer in Electron; preserved verdict at [`2026-06-23-perf-wave/SLICE-002.md`](2026-06-23-perf-wave/SLICE-002.md)). One squash PR #219 (`64c04f3b`); full e2e matrix both legs ×2 (Win 180 / Linux 180). Raw cards + FIX-REPORT + queue collapsed to git history (`git log --all -- perf-slices/`). |
| gitdiff completeness audit (2026-06-23) | [`2026-06-23-gitdiff-audit/`](2026-06-23-gitdiff-audit/) — single-feature completeness audit of `gitDiff` (read-only working-tree diff → Command-board result zone + `git_diff` MCP tool). Intent spec + impl-vs-intent coverage matrix + diagnoses (all existing tests/e2e reproduced **green** — the "failing tests" premise was stale) → **7 gaps** (2 Med · 5 Low; 0 blocking). **✅ COMPLETE** — all 7 fixed on `fix/gitdiff-gaps` via a file-disjoint workflow (untracked-file inclusion read-only · MAIN-side read bound + abort/timeout · hunk-aware diffstat · stale `git_diff`-tool comments reconciled · real-git test coverage · scope-label · renderer timeout), gate green (typecheck · lint · 3286 unit · gitDiff e2e 3/3), adversarial review APPROVE 0 must-fix. |
