...
- **Status:** **PHASE A COMPLETE.** A1 (preset dialog + brand glyphs + identity + schema v10), A2
  (searchable per-agent command builder with category tabs), and the **unified create+edit dialog
  (Option A)** — NewTerminalDialog now serves BOTH the place-first create flow and the gear-icon edit
  flow, and the old in-canvas TerminalConfig popover + its configDirty guard are removed. Gate green:
  typecheck/lint/format, unit+integration 2298, Windows e2e 111/111. Rebased onto main `5fbb7b0`.
  **Phase B (MCP agent-identity observation) is the remaining follow-up; the full e2e matrix (Linux
  leg) is owed at the pre-merge gate.**
